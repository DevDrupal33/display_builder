...
Drupal.displayBuilder.initDrawer = (builder, debug) => {
  ...
  const clearSecondDrawerOpenedFlag = () => {
    document.body
   ...
   *   The current variant of the trigger button (e.g., 'default', 'primary').
   */
  const handleSecondDrawerTriggerClick = (trigger) => {
    clearSecondDrawerOpenedFlag();

    if (secondDrawer?.open && trigger === activeSecondDrawerButton) {
      if (debug) console.debug('[drawer] second Drawer open: hide');
      secondDrawer.hide();
      trigger.variant = 'default';
      activeSecondDrawerButton = null;
    } else {
      if (debug) console.debug('[drawer] second Drawer hidden: open');
      secondDrawer.show();
      trigger.variant = 'primary';
      if (activeSecondDrawerButton)
        activeSecondDrawerButton.variant = 'default';
  ...
  // builder.addEventListener('htmx:load', (event) => {
    // const triggerSecondDrawer = event.detail.elt.querySelectorAll(
    //   '[data-open-second-drawer]',
    // );
    // if (triggerSecondDrawer.length === 0) {
    //   return;
    // }

    // if (debug)
    //   console.log(
    //     `[drawer] Found ${triggerSecondDrawer.length} opener for second drawer`,
    //   );
    // triggerSecondDrawer.forEach((newTrigger) => {
    //   newTrigger.addEventListener('click', () => {
    //     handleSecondDrawerTriggerClick(newTrigger);
    //   });
    // });
  // });

  // After swap we need to keep the active mark.
  // @todo could be added to the result of the controller directly?
  builder.addEventListener('htmx:afterSwap', (event) => {
    if (!event.detail.elt.dataset?.instanceId) {
      return;
    }
    const isOpenDrawer = event.detail.elt.dataset?.openSecondDrawer;
    if (!isOpenDrawer) {
      return;
    }

    if (debug)
      console.log(`[drawer] Found swapped opener for second drawer`);

    event.detail.elt.setAttribute('data-drawer-trigger-opened', true);
    event.detail.elt.addEventListener('click', () => {
      handleSecondDrawerTriggerClick(event.detail.elt);
    });
  });
