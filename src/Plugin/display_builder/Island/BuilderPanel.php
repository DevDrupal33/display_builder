...
class BuilderPanel extends IslandPluginBase {
  ...
  protected function buildSingleComponent(string $builder_id, string $instance_id, SourceWithSlotsInterface $source, array $data, int $index = 0): ?array {
    ...
    $build['#attributes']['data-node-title'] = $source->label();
    $build['#attributes']['data-slot-position'] = $index;

    foreach ($source->getSlotDefinitions() as $slot_id => $definition) {
      $slot = $this->buildComponentSlot($builder_id, $source, $slot_id, $definition, $data, $instance_id);
  ...
  protected function replaceInstance(string $builder_id, string $instance_id): array {
    ...
    $source = $this->sourceManager->createInstance($data['source_id'], $data['source']);
  ...
  protected function digFromSlot(string $builder_id, array $data): array {
    ...
    foreach ($data as $index => $source) {
      ...
      $source_plugin = $this->sourceManager->createInstance($source['source_id'], $source['source'] ?? []);
   ...
   */
  private function buildComponentSlot(string $builder_id, SourceWithSlotsInterface $source, string $slot_id, array $definition, array $data, string $instance_id): array {
    $dropzone = [
      '#type' => 'component',
      '#component' => 'display_builder:dropzone',
      '#props' => [
    ...
    if (isset($source->getSlotValues()[$slot_id])) {
      $sources = $source->getSlotValues()[$slot_id];
